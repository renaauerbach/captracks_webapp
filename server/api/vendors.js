  ...
  if (req.isAuthenticated()) {
    ...
    if (req.body.linkUrl) {
      update = {
        $push: {
          links: {
            $each: [{ title: req.body.linkTitle, url: req.body.linkUrl }],
            $position: 0,
          },
        },
      };
    ...
    .exec((err, store) => {
      if (err || !store) {
        ...
        if (currentCapcity > store.details[0].maxCapacity) {
          ...
          res.redirect("/map/store/" + store._id);
        }
      }
    });
});
      ...
      if (err || !store) {
        ...
        if (currentCapcity > 0) {
          ...
          newdetails.save((err) => {
            ...
            if (err) {
              console.log(
                "Save Error while invoking exit detail model :" + err
              );
              return res.status(400).send(err);
            }
          });
        ...
        res.redirect("/map/store/" + store._id + "/exit");
      }
    });
});
      ...
      .exec((err, store) => {
        {
          ...
          doc.addImage(store.vendor.exitqrcode, "PNG", 30, 90, 150, 150);
          doc.setFontSize(20);
          doc.text(
            doc.internal.pageSize.getWidth() / 2,
